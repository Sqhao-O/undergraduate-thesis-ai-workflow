...
| 顺序 | 文档 | 你将学会 |
|:---:|------|----------|
| 第 1 篇 | README（本文） | 项目全貌、快速开始入口 |
| 第 2 篇 | Claude Code 配置 | 安装开发环境 |
| 第 3 篇 | CC Switch 配置 | 配置 API 密钥（可选） |
| 第 4 篇 | Pandoc 使用 | 把 Markdown 转成 Word |
| 第 5 篇 | WorkBuddy 排版 | AI 精细排版配置 |
| 第 6 篇 | 完整工作流 | **整合所有工具的 8 阶段流程** |

**阅读时间：** 约 30 分钟可读完所有文档

> 提示：点击下方「开始阅读」按钮，从第 2 篇开始即可！
...
**本工作流是开源文档性质，AI 仅作辅助工具，不提倡、不支持任何形式的学术造假，使用者需严格遵守所在院校学术规范，一切违规使用导致的学术责任由使用者自行承担。**
...
| 顺序 | 工具 | 是否必须 | 下载地址 | 用途 |
|------|------|---------|----------|------|
| **1** | **Git for Windows** | **必装！** | https://git-scm.com/downloads/win | Claude Code 内部使用 Git Bash 执行命令 |
| **2** | **Node.js** | **必须装** | https://nodejs.org/ | Claude Code 的运行环境 |

> Windows 用户特别注意：没有 Git，Claude Code 根本跑不起来！
...
> 注意：JSON 格式非常严格，必须使用英文标点符号，中文逗号或引号会导致错误！

---

## 6. 核心工具介绍

### 6.1 1. CC Switch — 配置管理神器
...
| OpenClaw | 另一款 AI 编程助手 |
| Gemini CLI | Google 的 AI 编程工具 |

#### 6.1.2 主要功能

- 添加 Provider：支持 Anthropic、OpenAI、Google、DeepSeek 等多种 API 服务商
- 一键切换 API 配置：快速在不同的 API Key 之间切换
- 管理 MCP 服务器：配置 Model Context Protocol 服务扩展
- 管理 Skills：管理 AI 工具的技能插件
- 系统托盘常驻：最小化到系统托盘，随时快速切换
...
### 6.4 4. WorkBuddy（腾讯小龙虾）— 智能精细排版

**英文名称：** WorkBuddy

WorkBuddy 是腾讯推出的 AI 助手平台，其中集成了**论文智能排版 Skill**，可以对论文进行精细化的格式检查和自动修正。

#### 6.4.1 Paper Check Skill
...
**B站教程视频：** 【开源毕业论文一键排版工具 Lark-Formatter V0.2 LTS】
https://b23.tv/aBf4B83

#### 6.4.3 详细使用教程

<a href="docs/04-workbuddy-guide.md" style="display: inline-block; padding: 6px 16px; background-color: #2ea44f; color: white; text-decoration: none; border-radius: 4px; font-weight: bold; font-size: 12px;">查看详细使用教程</a>

---

## 7. 完整工作流步骤

> **核心章节！** 下面是从零开始完成本科毕业论文的完整流程概览。
> **详细步骤说明、工具配置、时间规划等内容，点击下方链接查看完整工作流文档：
> <a href="docs/05-workflow-guide.md" style="display: inline-block; padding: 6px 16px; background-color: #0969da; color: white; text-decoration: none; border-radius: 4px; font-weight: bold; font-size: 12px;">📋 完整工作流文档（第 6 篇）</a>**
...
#### 7.2.2 步骤二：用 Claude Code 进行开发与仿真

1. 打开终端，进入你的项目文件夹
2. 启动 Claude Code：`claude`
3. 把需求文档的内容粘贴给 Claude Code
4. 让它读取工程文件、编写代码、运行调试

##### 7.2.2.1 Claude Code 常用指令
...
#### 7.2.3 步骤三：编写 Markdown 格式的论文初稿

1. 使用 VS Code、Typora 或任何 Markdown 编辑器
2. 按照学校论文格式要求编写章节
3. 使用标准的 Markdown 语法：`# 一级标题`、`## 二级标题` 等
4. 插入图片、表格、公式等

#### 7.2.4 步骤四：用 Pandoc 转换为 DOCX
...
#### 7.2.6 步骤五：用 Lark-Formatter 初步排版
...
> 更多细节：<a href="docs/02-cc-switch-guide.md" style="display: inline-block; padding: 6px 16px; background-color: #2ea44f; color: white; text-decoration: none; border-radius: 4px; font-weight: bold; font-size: 12px;">CC Switch 配置密钥</a> <a href="docs/05-workflow-guide.md" style="display: inline-block; padding: 6px 16px; background-color: #2ea44f; color: white; text-decoration: none; border-radius: 4px; font-weight: bold; font-size: 12px;">完整工作流图解</a>
...
└── docs/
    ...
    ├── 04-workbuddy-guide.md    # WorkBuddy 排版教程
    └── 05-workflow-guide.md     # 完整工作流图解（整合所有工具）
```
...
<p align="center">
  <strong>再次强调：AI 是工具，不是替代品。请诚实做人，认真做学问。</strong>
</p>
...
> **按推荐顺序依次阅读，点击链接即可跳转到下一篇，无需手动切换文件**

### 10.1 阅读路线图

```
┌─────────────────┐
│  README.md      │ ← 你在这里！（第 1 篇，共 6 篇）
│    主文档        │
└────────┬────────┘
         │ 点击开始
         ▼
┌───────────────────────────────────────────────────────┐
│  第 2 篇：01-claude-code-setup.md                      │
│  （Git安装 + Node.js + Claude Code配置 + 跳过登录）      │
└────────┬───────────────────────────────────────────────┘
         │
         ▼
┌───────────────────────────────────────────────────────┐
│  第 3 篇：02-cc-switch-guide.md                         │
│  （CC Switch下载 + API密钥配置 + Provider切换）           │
└────────┬───────────────────────────────────────────────┘
         │
         ▼
┌───────────────────────────────────────────────────────┐
│  第 4 篇：03-pandoc-usage.md                            │
│  （Pandoc安装验证 + Markdown→DOCX转换详解）              │
└────────┬───────────────────────────────────────────────┘
         │
         ▼
┌───────────────────────────────────────────────────────┐
│  第 5 篇：04-workbuddy-guide.md                          │
│  （WorkBuddy安装 + Paper Check Skill + 自动排版实战）     │
└────────┬───────────────────────────────────────────────┘
         │
         ▼
┌───────────────────────────────────────────────────────┐
│  第 6 篇：05-workflow-guide.md                          │
│  （完整工作流8阶段图解 + 时间规划 + 工具协作关系）★       │
└────────┴───────────────────────────────────────────────┘
         │ 完成！回到开头
         ▼
     （返回 README.md）
...
| # | 文档 | 内容摘要 | 链接 |
|---|------|----------|------|
| **1** | **README.md** | 项目总览、基础概念、快速开始、工具介绍、工作流步骤 | 当前页面 |
| **2** | `01-claude-code-setup.md` | Git 安装 / Node.js 安装 / Claude Code 安装与验证 / 跳过登录 / 常用指令 | <a href="./docs/01-claude-code-setup.md" style="display: inline-block; padding: 10px 20px; background-color: #2ea44f; color: white; text-decoration: none; border-radius: 6px; font-weight: bold; font-size: 14px;">开始阅读</a> |
| **3** | `02-cc-switch-guide.md` | CC Switch 下载安装 / 添加 Provider / 配置 API 密钥 / MCP 管理 | <a href="./docs/02-cc-switch-guide.md" style="display: inline-block; padding: 10px 20px; background-color: #2ea44f; color: white; text-decoration: none; border-radius: 6px; font-weight: bold; font-size: 14px;">开始阅读</a> |
| **4** | `03-pandoc-usage.md` | Pandoc 安装验证 / 支持格式列表 / 转换命令 / Word模板使用 / 中文处理 | <a href="./docs/03-pandoc-usage.md" style="display: inline-block; padding: 10px 20px; background-color: #2ea44f; color: white; text-decoration: none; border-radius: 6px; font-weight: bold; font-size: 14px;">开始阅读</a> |
| **5** | `04-workbuddy-guide.md` | WorkBuddy 安装 / Paper Check Skill / 自动排版实战 / 对比报告 | <a href="./docs/04-workbuddy-guide.md" style="display: inline-block; padding: 10px 20px; background-color: #2ea44f; color: white; text-decoration: none; border-radius: 6px; font-weight: bold; font-size: 14px;">开始阅读</a> |
| **6** | `05-workflow-guide.md` | 完整工作流 8 阶段图解 / Markdown写作规范 / 时间规划建议 | <a href="./docs/05-workflow-guide.md" style="display: inline-block; padding: 10px 20px; background-color: #2ea44f; color: white; text-decoration: none; border-radius: 6px; font-weight: bold; font-size: 14px;">开始阅读</a> |
